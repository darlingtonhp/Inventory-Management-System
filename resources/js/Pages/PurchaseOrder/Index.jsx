...
export default function Index({ auth, orders, success }) {
    ...
    const deletePO = (po) => {
        if (!window.confirm(`Are you sure you want to delete draft PO ${po.reference}?`)) {
            return;
        }
        router.delete(route('purchase-order.destroy', po.id));
    };
    ...
    return (
        <ModuleLayout currentModule="purchasing" breadcrumbs={crumbs}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-6 shadow-sm flex items-center justify-between flex-wrap gap-4">
                <div>
                    <h1 className="text-xl font-bold text-gray-900 font-display">Purchase Orders Directory</h1>
                    <p className="text-xs text-gray-500 mt-1">Track external vendor orders, delivery status, and receiving timelines</p>
                </div>
                <Link
                    href={route('purchase-order.create')}
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {orders.data.length > 0 ? (
                                orders.data.map((order) => {
                                    const total = getPoTotal(order);
                                    return (
                                        <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                                            {/* PO Reference */}
                                            <td className="px-6 py-4 font-bold text-gray-950 text-xs">
                                                <Link href={route('purchase-order.show', order.id)} className="hover:text-indigo-600">
                                                    {order.reference}
                                                </Link>
                                            </td>
                                            ...
                                            {/* Expected Date */}
                                            <td className="px-6 py-4 text-gray-500 text-xs">
                                                {order.expected_date || 'Not specified'}
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-center relative">
                                                <button
                                                    ...
                                                    className="w-8 h-8 rounded-lg bg-indigo-650 bg-indigo-600 hover:bg-indigo-700 text-white font-bold inline-flex items-center justify-center shadow-sm transition-colors focus:outline-none"
                                                >
                                                    ...
                                                </button>
                                                ...
                                                {activeMenuId === order.id && (
                                                    <>
                                                        ...
                                                        <div className="absolute right-6 mt-1 w-32 bg-white border border-gray-200 rounded-lg shadow-lg py-1 z-50 text-left">
                                                            <Link
                                                                href={route('purchase-order.show', order.id)}
